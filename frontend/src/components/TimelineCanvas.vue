...
<template>
  <div ref="wrapperRef" class="relative h-full w-full select-none overflow-hidden">
    ...
    <TooltipCard
      v-if="hoveredEvent && tooltipPos"
      :event="hoveredEvent"
      :x="tooltipPos.x"
      :y="tooltipPos.y"
    />

    <div v-if="store.error" class="absolute bottom-3 left-3 z-30 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-zinc-200">
