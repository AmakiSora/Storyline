...
export function useTimeScale(params: {
  ...
  const ticks = computed(() => {
    ...
    const ticks: Array<{ x: number; label: string; kind: 'year' | 'month' | 'day' }> = []

    if (kind === 'year') {
      const fromDate = new Date(fromMs)
      const start = startOfYearUtcMs(fromDate)
      const yearsSpan = Math.max(1, new Date(toMs).getUTCFullYear() - new Date(fromMs).getUTCFullYear() + 1)
      const step = Math.max(1, Math.ceil(yearsSpan / targetCount))

      for (let t = start; t <= toMs + 24 * 60 * 60 * 1000; t = addYearsUtc(t, step)) {
        const x = timeToX(t)
        if (x < -120 || x > w + 120) continue
        ticks.push({ x, label: formatYear(t), kind })
      }
      return ticks
    }
    ...
    if (kind === 'month') {
      ...
      for (let t = start; t <= toMs + 24 * 60 * 60 * 1000; t = addMonthsUtc(t, step)) {
        const x = timeToX(t)
        if (x < -120 || x > w + 120) continue
        ticks.push({ x, label: formatMonth(t), kind })
      }
      return ticks
    }
    ...
    for (let t = start; t <= toMs + 24 * 60 * 60 * 1000; t = addDaysUtc(t, step)) {
      const x = timeToX(t)
      if (x < -120 || x > w + 120) continue
      ticks.push({ x, label: formatDay(t), kind })
    }
